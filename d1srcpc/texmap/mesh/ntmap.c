...
// 	div_numerator = DivNum;	//f1_0*3;
	...
	if ( Tmap_file == NULL ) {
		Tmap_file = fopen( "mesh.out", "wt" );
		atexit(close_tmap_file);
	}
	fprintf( Tmap_file, "{ %d ", nverts );

	for (i=0; i<nverts; i++) {
		g3ds_vertex	*tvp = &Tmap1.verts[i];
		g3s_point	*vp = vertbuf[i];

		fprintf( Tmap_file, ",0x%x,0x%x,0x%x ", vp->p3_vec.x, vp->p3_vec.y, vp->p3_vec.z );
