/*
 * $Source: f:/miner/source/main/editor/rcs/med.c $
 * $Revision: 2.3 $
 * $Author: john $
 * $Date: 1995/03/06 18:23:52 $
 *
...
void medkey_init()
{
	...
	//MALLOC( LispCommand, char, DIAGNOSTIC_MESSAGE_MAX );//KRB hack
...
int SetPlayerFromCursegMinusOne()
{
	...
	for (i=max=0;i<4;i++) {
		...
		if (labs(corner_p[i].x) > max) max = labs(corner_p[i].x);
		if (labs(corner_p[i].y) > max) max = labs(corner_p[i].y);
	}

	view_dist = fixmul(view_dist,fixdiv(fixdiv(max,SIDE_VIEW_FRAC),corner_p[0].z));
	vm_vec_copy_scale(&view_vec2,&view_vec,view_dist);
	vm_vec_sub(&ConsoleObject->pos,&side_center,&view_vec2);
