...
*/
/*
 * $Source: f:/miner/source/main/editor/rcs/segment.c $
 * $Revision: 2.0 $
 * $Author: john $
 * $Date: 1995/02/27 11:35:21 $
 *
...
int check_for_degenerate_side(segment *sp, int sidenum)
{
	...
	vm_vec_normalized_dir(&vec1, &Vertices[sp->verts[vp[1]]], &Vertices[sp->verts[vp[0]]]);
	vm_vec_normalized_dir(&vec2, &Vertices[sp->verts[vp[2]]], &Vertices[sp->verts[vp[1]]]);
	...
	//vm_vec_normalize(&vec1);
	//vm_vec_normalize(&vec2);
	vm_vec_normalized_dir(&vec1, &Vertices[sp->verts[vp[2]]], &Vertices[sp->verts[vp[1]]]);
	vm_vec_normalized_dir(&vec2, &Vertices[sp->verts[vp[3]]], &Vertices[sp->verts[vp[2]]]);
	vm_vec_cross(&cross, &vec1, &vec2);
...
//	destseg->sides[destside].render_flag = 0;
	...
	for (v=0; v<4; v++)
		nsp->verts[v] = destseg->verts[dvp[v]];
...
void scale_free_vertices(segment *sp,vms_vector *vp,fix scale_factor,int min_side,int max_side)
{
	...
	for (i=0; i<4; i++)
		if (is_free_vertex(sp->verts[verts[i]])) {
			Vertices[sp->verts[verts[i]]].x = fixmul(vp->x,scale_factor)/2;
			Vertices[sp->verts[verts[i]]].y = fixmul(vp->y,scale_factor)/2;
			Vertices[sp->verts[verts[i]]].z = fixmul(vp->z,scale_factor)/2;
		}
...
int med_form_joint(segment *seg1, int side1, segment *seg2, int side2)
{
	...
	for (v=0; v<4; v++)
		lost_vertices[v] = seg2->verts[vp2[v]];
...
// mprintf((0,"Forming bridge segment %i from %i to %i\n",bs-Segments,seg1-Segments,seg2-Segments));
	...
	for (v=0; v<4; v++)
		bs->verts[(3-v)+4] = seg2->verts[sv[v]];
	...
	for (v=0; v<4; v++)
		bs->verts[(v + bfi) % 4] = seg1->verts[sv[v]];
	...
	bs->children[Side_opposite[AttachSide]] = seg2 - Segments;
	...
	if (Degenerate_segment_found) {
		...
		bs->children[Side_opposite[AttachSide]] = -1;
